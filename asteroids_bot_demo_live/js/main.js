...
Game.prototype.display = function () {
    ...
    for (var i in this.ships) {
        if (this.ships[i].alive) {
            ...
            this.ctx.strokeRect(
                this.ships[i].x, this.ships[i].y,
                this.ships[i].width, this.ships[i].height);
            this.ctx.drawImage(
                images.ship, this.ships[i].x, this.ships[i].y,
                this.ships[i].width, this.ships[i].height);
        }
    }
    ...
    for (var i in this.asteroids) {
        this.ctx.strokeRect(
            this.asteroids[i].x, this.asteroids[i].y,
            this.asteroids[i].width, this.asteroids[i].height);
        this.ctx.drawImage(
            images.asteroid, this.asteroids[i].x, this.asteroids[i].y,
            this.asteroids[i].width, this.asteroids[i].height);
    }
...
Ship.prototype.getSensorDistances = function () {
    ...
    for (var i in game.asteroids) {
        var distance = Math.sqrt(Math.pow(
            game.asteroids[i].x + game.asteroids[i].width / 2 - this.x + this.width / 2, 2) +
            Math.pow(game.asteroids[i].y + game.asteroids[i].height / 2 -
        ...
        if (distance <= maxSensorSize) {
            for (var j = 0; j < nbSensors; j++) {
                ...
                if (Math.abs(Math.atan2(objy - y1, objx - x1) - Math.atan2(
                    y2 - y1, x2 - x1)) <= Math.PI * 2 / nbSensors) {
                    var d = collisionSegmentAABB(
                        x1, y1, x2, y2, game.asteroids[i].x, game.asteroids[i].y,
                        game.asteroids[i].width, game.asteroids[i].height);
